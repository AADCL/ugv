...
- 全局和局部 costmap 膨胀半径由 `0.25 m` 调整为 `0.22 m`，保持 40 cm 车体加 2 cm padding 的包络，用于低速验证 60 cm 门洞。
...
└── docs/
    ├── 轮趣四轮差速机器人_开发实施文档_V5.0.md
    ├── 轮趣四轮差速机器人_使用文档_V5.0.md
    └── 轮趣四轮差速机器人_详细信息表_V5.0.md
```
...
完整的逐文件复制、修改、编译和验收流程见 `docs/轮趣四轮差速机器人_开发实施文档_V5.0.md`；日常命令见使用文档，全部参数见详细信息表。V5.0 正式链路为 PGM 唯一静态占据、保存高程坡度代价、实时高程相对障碍、GlobalPlanner 与 TEB，不提供 DWA；当前目标是平地和连续坡道路段，楼梯语义不在本版本验收范围。
