...
namespace
{
std::unique_lock<std::mutex> AcquirePrioritySerialWriteLock(
    std::mutex& serial_mutex,
    std::atomic<unsigned int>& pending_writes)
{
    ...
    const ros::MessageEvent<geometry_msgs::Twist const>& event)
{
  ...
  if (!Cmd_Source_Guard.Accept(connection_header.get()))
  {
    ...
    ROS_ERROR_THROTTLE(
        1.0,
        "Rejected chassis command from publisher '%s'; expected '%s'. "
        "Source fault is latched until driver restart and output is locked",
        caller.c_str(), wheeltec_driver_safety::kExpectedCmdVelCallerId);
    return;
  }

  const geometry_msgs::Twist::ConstPtr message = event.getMessage();
  if (!message)
  {
    Cmd_Watchdog.Lock();
    ROS_ERROR_THROTTLE(1.0,
                       "Rejected empty chassis command and locked output");
    return;
  }
  const geometry_msgs::Twist& twist_aux = *message;

  if (!std::isfinite(twist_aux.linear.x) ||
      !std::isfinite(twist_aux.linear.y) ||
      !std::isfinite(twist_aux.angular.z))
  {
    Cmd_Watchdog.Lock();
    ROS_ERROR_THROTTLE(1.0, "Rejected non-finite cmd_vel and locked output");
    return;
  }

  const geometry_msgs::Twist limited =
      LimitCmdVel(twist_aux, max_linear_x, max_linear_y, max_angular_z);
  if (limited.linear.x != twist_aux.linear.x ||
      limited.linear.y != twist_aux.linear.y ||
      limited.angular.z != twist_aux.angular.z)
  {
    ROS_WARN_THROTTLE(1.0,
                      "cmd_vel limited to x=+/-%.2f, y=+/-%.2f, yaw=+/-%.2f",
                      max_linear_x, max_linear_y, max_angular_z);
  }

  // The callback only renews the command lease. A fixed-rate wall timer owns
  // every serial write so a publisher crash cannot leave a stale command set.
...
void turn_on_robot::Command_Timer_Callback(const ros::WallTimerEvent&)
{
  ...
  {
    // Select the command only after acquiring the serial write order. Even if
    // timer concurrency is changed later, an older non-zero snapshot cannot
    // be written after a newer timeout-generated zero.
  ...
  const bool was_fresh = Last_Command_Was_Fresh.exchange(fresh);
  if (was_fresh && !fresh)
    ROS_WARN("cmd_vel timed out after %.3f s; forcing zero velocity",
             cmd_vel_timeout);
    ...
    const geometry_msgs::Twist& command)
{
  ...
  const auto store_velocity = [&packet, &scaled_velocity](std::size_t high,
                                                          double velocity) {
  ...
  try
  {
    ...
    {
      ROS_ERROR_THROTTLE(1.0, "Cannot send chassis command: serial port is closed");
      return false;
    }
    const size_t bytes_written =
        Stm32_Serial.write(packet.data(), packet.size());
    if (bytes_written != packet.size())
    {
      ROS_ERROR_THROTTLE(1.0,
                         "Incomplete chassis command write; output locked");
      return false;
    }
  }
  catch (const serial::IOException&)
  {
    ROS_ERROR_THROTTLE(1.0, "Unable to send chassis command through serial port");
    return false;
  }
  catch (const std::exception& error)
  {
    ROS_ERROR_THROTTLE(1.0, "Chassis serial write failed: %s", error.what());
    return false;
  }
  return true;
}

void turn_on_robot::Send_Stop_Burst(int repeat_count)
{
  const geometry_msgs::Twist stop;
  for (int i = 0; i < repeat_count; ++i)
  {
    Send_Velocity_Command(stop);
    if (i + 1 < repeat_count)
      ros::WallDuration(stop_repeat_interval).sleep();
  }
}

size_t turn_on_robot::Read_Serial(uint8_t* buffer, size_t size)
{
  if (Pending_Serial_Writes.load(std::memory_order_acquire) != 0)
    return 0;

  std::unique_lock<std::mutex> lock(Serial_Mutex, std::try_to_lock);
  if (!lock.owns_lock())
    return 0;

  // Close the race where a writer announced itself after the first check but
  // before this reader acquired the mutex. Once announced, new reads yield.
  if (Pending_Serial_Writes.load(std::memory_order_acquire) != 0)
    return 0;
  if (!Stm32_Serial.isOpen())
    return 0;
  if (Stm32_Serial.available() < size)
    return 0;
  return Stm32_Serial.read(buffer, size);
}
...
bool turn_on_robot::Get_Sensor_Data()
{
  ...
  {
    if (Read_Serial(Receive_Data_Pr, sizeof(Receive_Data_Pr)) !=
        sizeof(Receive_Data_Pr))
      return false;
  }
  else if (flag_error==1) //Error condition detected 检测到错误情况
  {
    ...
    if (Read_Serial(Receive_Data_Tr, sizeof(Receive_Data_Tr)) !=
        sizeof(Receive_Data_Tr))
      return false;
...
{
  if (!Initialization_Succeeded.load())
  {
    ROS_FATAL("Chassis driver initialization failed; control loop will not start");
    return;
  }

  // One callback worker serializes cmd_vel updates and wall-timer callbacks.
  // Serial reception remains independent in this Control() thread.
  ...
  while(ros::ok())
  {
    ...
    {
      ROS_ERROR("Serial read failed; stopping the chassis command loop");
      ros::shutdown();
      break;
    }
    catch (const std::exception& error)
    {
      ROS_ERROR("Serial read failed: %s; stopping the chassis command loop",
                error.what());
      ros::shutdown();
      break;
    }
  ...
  // Stop callback execution before the final zero burst. This prevents an
  // already queued timer callback from writing a stale non-zero command after
...
turn_on_robot::turn_on_robot():Sampling_Time(0),Power_voltage(0)
{
  ...
  std::string requested_cmd_vel_topic;
  if (private_nh.getParam("cmd_vel_topic", requested_cmd_vel_topic) &&
      requested_cmd_vel_topic != wheeltec_driver_safety::kDriverCmdVelTopic)
  {
    ROS_ERROR("Ignoring unsafe cmd_vel_topic override '%s'; physical chassis "
              "input is fixed at '%s'",
              requested_cmd_vel_topic.c_str(),
              wheeltec_driver_safety::kDriverCmdVelTopic);
  }
  const std::string resolved_cmd_vel_topic =
      n.resolveName(wheeltec_driver_safety::kDriverCmdVelTopic);
  if (!wheeltec_driver_safety::IsInvariantDriverTopic(
          resolved_cmd_vel_topic))
  {
    ROS_FATAL("Rejected ROS name remap of physical chassis input '%s' to '%s'",
              wheeltec_driver_safety::kDriverCmdVelTopic,
              resolved_cmd_vel_topic.c_str());
    ros::shutdown();
    return;
  }
  private_nh.param<double>("cmd_vel_timeout", cmd_vel_timeout,
                           wheeltec_driver_safety::kDefaultCmdVelTimeout);
  private_nh.param<double>("command_publish_rate", command_publish_rate,
                           wheeltec_driver_safety::kDefaultCommandPublishRate);
  private_nh.param<int>("serial_read_timeout_ms", serial_read_timeout_ms, 10);
  private_nh.param<int>("startup_stop_repeats", startup_stop_repeats, 5);
  private_nh.param<int>("shutdown_stop_repeats", shutdown_stop_repeats, 5);
  private_nh.param<double>("stop_repeat_interval", stop_repeat_interval, 0.02);
  ...
  const double requested_cmd_vel_timeout = cmd_vel_timeout;
  const double requested_command_publish_rate = command_publish_rate;
  const int requested_serial_read_timeout_ms = serial_read_timeout_ms;
  const int requested_startup_stop_repeats = startup_stop_repeats;
  const int requested_shutdown_stop_repeats = shutdown_stop_repeats;
  const double requested_stop_repeat_interval = stop_repeat_interval;
  const double requested_max_linear_x = max_linear_x;
  const double requested_max_linear_y = max_linear_y;
  const double requested_max_angular_z = max_angular_z;

  cmd_vel_timeout = wheeltec_driver_safety::ClampFiniteParameter(
      cmd_vel_timeout, wheeltec_driver_safety::kDefaultCmdVelTimeout,
      wheeltec_driver_safety::kMinCmdVelTimeout,
      wheeltec_driver_safety::kMaxCmdVelTimeout);
  command_publish_rate = wheeltec_driver_safety::ClampFiniteParameter(
      command_publish_rate,
      wheeltec_driver_safety::kDefaultCommandPublishRate,
      wheeltec_driver_safety::kMinCommandPublishRate,
      wheeltec_driver_safety::kMaxCommandPublishRate);
  serial_read_timeout_ms = wheeltec_driver_safety::ClampIntegerParameter(
      serial_read_timeout_ms, wheeltec_driver_safety::kMinSerialTimeoutMs,
      wheeltec_driver_safety::kMaxSerialTimeoutMs);
  startup_stop_repeats = wheeltec_driver_safety::ClampIntegerParameter(
      startup_stop_repeats, wheeltec_driver_safety::kMinStopRepeats,
      wheeltec_driver_safety::kMaxStopRepeats);
  shutdown_stop_repeats = wheeltec_driver_safety::ClampIntegerParameter(
      shutdown_stop_repeats, wheeltec_driver_safety::kMinStopRepeats,
      wheeltec_driver_safety::kMaxStopRepeats);
  stop_repeat_interval = wheeltec_driver_safety::ClampFiniteParameter(
      stop_repeat_interval, 0.02, 0.0,
      wheeltec_driver_safety::kMaxStopRepeatInterval);
  max_linear_x = wheeltec_driver_safety::ClampFiniteParameter(
      max_linear_x, wheeltec_driver_safety::kMaxLinearX, 0.0,
      wheeltec_driver_safety::kMaxLinearX);
  max_linear_y = 0.0;
  max_angular_z = wheeltec_driver_safety::ClampFiniteParameter(
      max_angular_z, wheeltec_driver_safety::kMaxAngularZ, 0.0,
      wheeltec_driver_safety::kMaxAngularZ);

  const bool parameters_adjusted =
      requested_cmd_vel_timeout != cmd_vel_timeout ||
      requested_command_publish_rate != command_publish_rate ||
      requested_serial_read_timeout_ms != serial_read_timeout_ms ||
      requested_startup_stop_repeats != startup_stop_repeats ||
      requested_shutdown_stop_repeats != shutdown_stop_repeats ||
      requested_stop_repeat_interval != stop_repeat_interval ||
      requested_max_linear_x != max_linear_x ||
      requested_max_linear_y != max_linear_y ||
      requested_max_angular_z != max_angular_z;
  if (parameters_adjusted)
  {
    ROS_WARN("Unsafe or invalid driver parameters were clamped to hard "
             "safety bounds");
  }
  Cmd_Watchdog.SetTimeout(cmd_vel_timeout);
  ...
  Command_Timer = n.createWallTimer(
      ...
      &turn_on_robot::Command_Timer_Callback, this, false, false);

  ROS_INFO("Command watchdog ready: input=%s timeout=%.3f s, output_rate=%.1f Hz",
           wheeltec_driver_safety::kDriverCmdVelTopic, cmd_vel_timeout,
           command_publish_rate);
  ROS_INFO("Only cmd_vel publisher '%s' is accepted",
           wheeltec_driver_safety::kExpectedCmdVelCallerId);
  ROS_INFO("Final cmd_vel limits: x=+/-%.2f m/s, y=+/-%.2f m/s, "
           "yaw=+/-%.2f rad/s",
           max_linear_x, max_linear_y, max_angular_z);
  ...
  try
  {
    ...
    serial::Timeout _time =
        serial::Timeout::simpleTimeout(serial_read_timeout_ms); //Timeout //超时等待
  ...
  {
    ROS_ERROR("wheeltec_robot cannot open serial port: %s", error.what());
  }
  if(Stm32_Serial.isOpen())
  {
    ...
    ROS_INFO("Startup interlock active; waiting for a fresh cmd_vel");
  }
  else
  {
    ROS_FATAL("wheeltec_robot cannot run without an open chassis serial port");
    ros::shutdown();
  }
}
/**************************************
...
{
  // Control() also sends a burst. Repeating it here covers early returns and
  // orderly destruction before Control() starts.
  Begin_Shutdown();
  std::unique_lock<std::mutex> lock = AcquirePrioritySerialWriteLock(
      Serial_Mutex, Pending_Serial_Writes);
  if (Stm32_Serial.isOpen())
